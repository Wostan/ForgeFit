...
public class DiaryService(IApiService apiService) : IDiaryService
{
    ...
    public async Task<ServiceResponse<List<FoodEntryDto>>> GetEntriesByDateRangeAsync(DateTime from, DateTime to,
        CancellationToken cancellationToken = default)
    {
        var fromStr = from.ToString("yyyy-MM-ddTHH:mm:ss");
        var toStr = to.ToString("yyyy-MM-ddTHH:mm:ss");
        return await apiService.GetAsync<List<FoodEntryDto>>(
            $"/api/food-tracking/entries/by-date?from={fromStr}&to={toStr}", cancellationToken);
    }

    public async Task<ServiceResponse<FoodEntryDto?>> GetEntryAsync(Guid entryId,
        CancellationToken cancellationToken = default)
    {
    ...
    {
        return await apiService.PutAsync<FoodEntryCreateRequest, FoodEntryDto>($"/api/food-tracking/entries/{entryId}",
            request);
    }

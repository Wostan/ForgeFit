    ...
    IAlertService alertService) : ObservableObject
{
    ...
    private async Task SaveFood()
    {
        ...
        if (!double.TryParse(normalizedInput, NumberStyles.Any, CultureInfo.InvariantCulture, out var amount) || amount <= 0)
        {
    ...
    public async Task OpenFoodDetailsInternal(FoodProductResponse productDetails, FoodSearchResponse sourceItem, bool isShowingRecent)
    {

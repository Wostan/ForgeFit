...
public partial class ExerciseSessionViewModel : ObservableObject
{
    ...
    public bool HasExercisesWithoutSets() => Exercises.Any(e => e.Sets.Count == 0);

    public List<PerformedExerciseDto> GetPerformedExercises()
    {
        return Exercises.Select(e => e.ToPerformedExerciseDto()).ToList();
    }

    public List<WorkoutExercisePlanDto> GetUpdatedPlans()
    {
        return Exercises.Select(e => new WorkoutExercisePlanDto(
            e.Id,
            _programId,
            e.WorkoutExercise,
